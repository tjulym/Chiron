...
def handle(req):
	...
	ps = []	
	for i, f in enumerate([upload_user_mentions, upload_creator, upload_text, upload_unique_id, upload_media]):
		ps.append(MyThread(runt, args=[f, compose_res]))	
	...
	ps = []
	for i, f in enumerate([upload_home_timeline, upload_user_timeline, post_storage]):
		ps.append(MyThread(runt, args=[f, compose_and_upload_res]))
